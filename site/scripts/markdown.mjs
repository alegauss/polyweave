...
// let the two drift. The nav and footer never reach a twin (dropped by tag) and the calls
// to action are dropped by their data-twin="omit" attribute.
...
// the block letter, the section kicker, terminal window bars and the file bar over a code
// block. Each of these is a glyph or a label that carries nothing in a flat file.
const SKIP_CLASSES = new Set([
  "dot", "chk", "ico", "eyebrow", "bar", "copy-btn", "block-letter", "block-count",
  "file-bar", "lattice",
]);
...
/** A roadmap line: the marker, the id, the symptom and the measurement behind it. */
function statusRow(node) {
  ...
  // the rest is everything in .what after its leading <b>
  const rest = what
    ? collapse(
        what.childNodes
...
/** A stat tile: a number and what it counts, which is one bullet and not two paragraphs. */
function statTile(node) {
  const value = node.querySelector(".stat-value")?.text.trim() ?? "";
  const label = node.querySelector(".stat-label")?.text.trim() ?? "";
  return value || label ? `- **${value}** — ${label}` : "";
}

/** A convention: the name, the decision, and the reason for it. */
function conventionTile(node) {
  const name = node.querySelector(".conv-name")?.text.trim() ?? "";
  const value = node.querySelector(".conv-value")?.text.trim() ?? "";
  const note = node.querySelector("p") ? inlineTrim(node.querySelector("p")) : "";
  return name ? `- **${name}**: ${value}${note ? ` — ${note}` : ""}` : "";
}
...
function blocks(node, out) {
  for (const child of node.childNodes) {
    ...
    } else if (cls.includes("stat")) {
      const t = statTile(child);
      if (t) out.push(t);
    } else if (cls.includes("conv")) {
      const t = conventionTile(child);
      if (t) out.push(t);
