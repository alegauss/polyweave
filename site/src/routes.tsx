...
import { claudeCode, meta, specs } from "./lib/site-content";
import { features } from "./lib/features";

// GitHub Pages derives the base from the repository name: the site is served at
// https://alegauss.github.io/polyweave/, so canonical, og:url, the sitemap and every
...
export type RouteMeta = {
  /** app path, leading slash: "/" or "/specs" or "/features/compiler" */
  path: string;
  title: string;
  description: string;
  ogTitle: string;
...
export const ROUTE_META: RouteMeta[] = [
  ...
  {
    path: "/specs",
    title: specs.meta.title,
    description: specs.meta.description,
    ogTitle: specs.meta.ogTitle,
    ogDescription: specs.meta.ogDescription,
  },
  // The depth pages' metadata is read off the same records as their routes and pages
  // below, so a pillar cannot ship with a route and no title, or a page and no route.
  ...features.map((f) => ({
...
/** The output path of a route's HTML file, relative to dist/. "/" → "", "/specs" → "specs". */
...
// with metadata but no component never gets a file. Both are silent at runtime, so they
// are made loud at import: this throw fails tsc/build/prerender, whichever imports first.
