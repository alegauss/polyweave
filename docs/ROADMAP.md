...
- 🛠 **PW76** (deps: —) **Cottony's four progress stars are lit by three constants found by hand, and no search sees them** — star_model builds the geometry from code, so the rig is the only hand-tuned part: the smallest honest port, and one that has to hold across two hues and two sizes. → §PW76
