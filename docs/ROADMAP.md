...
- ⏳ **PW76** (deps: PW84) **Cottony's four progress stars are lit by three constants found by hand, and no search sees them** — One rig holding on all four once lights scale with the subject, then the stars bake through it and a person judges the after side. → §PW76
...
- 📋 **PW84** (deps: —) **a rig found on a 96 px star goes dark and flat on the same star at 192 px** — Light power is stated in watts while the lights' size and distance scale with the subject, so a rig fitted at one size cannot hold at another. → §PW84
